...
def modify_data(source, freq_arr, flux_arr, eflux_arr, labels, sp_row, wise_row):
    ...
    '''             
    if final_morph == 'AX' and bx_morph != '-':
        # remove AX point
        ind = labels.index('AX')
        del labels[ind]
        del freq_arr[ind]
    ...
    if 'TGSS' in labels:
        ...
        flux = flux_arr[ind]
        eflux = eflux_arr[ind]
        eflux_arr[ind] = np.sqrt(eflux**2 + (flux*0.08)**2)
        
    if tgss_flag == 'U':
            labels.append('TGSS')
            flux_arr.append(2*wise_row['TGSS_Limit'])
            eflux_arr.append(wise_row['TGSS_Limit'])
            freq_arr.append(0.150)
    if vlssr_flag == 'U':
        noise = wise_row['VLSSr_Limit']
        if noise>0:
            labels.append('VLSSr')
            flux_arr.append(2*wise_row['VLSSr_Limit'])
            eflux_arr.append(wise_row['VLSSr_Limit'])
            freq_arr.append(0.074)
    if wenss_flag == 'U':
        noise = wise_row['WENSS_Limit']
        if noise>0:
            labels.append('WENSS')
            flux_arr.append(2*wise_row['WENSS_Limit'])
            eflux_arr.append(wise_row['WENSS_Limit'])
            freq_arr.append(0.325)
    if gleam_flag == 'U':
        noise = wise_row['GLEAM_Limit']
        if noise>0:
            labels.append('GLEAM')
            flux_arr.append(2*wise_row['GLEAM_Limit'])
            eflux_arr.append(wise_row['GLEAM_Limit'])
...
def mcsim_fitting(row, model, atgcat, vla_ax_grp, vla_bx_grp, ntrial):
    wisen = row['WISEname']
    scat = row
